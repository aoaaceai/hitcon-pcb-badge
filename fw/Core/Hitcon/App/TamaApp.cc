...
void TamaApp::Routine(void* unused) {
  ...
  switch (_state) {
    ...
    case TAMA_APP_STATE::PET_HEALING:
      needs_render = true;
      static_assert(TAMA_GET_ANIMATION_DATA(DOG_FED_HEALING).frame_count == TAMA_GET_ANIMATION_DATA(CAT_FED_HEALING).frame_count);
      if (_frame_count > TAMA_GET_ANIMATION_DATA(DOG_FED_HEALING).frame_count*3) {
        _state = TAMA_APP_STATE::IDLE;
        UpdateFrameBuffer();
      }
      break;
